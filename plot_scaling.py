...
def plot_speedup_efficiency(df: pd.DataFrame, serial_time: float, outpath: str):
    # Separate GPU row before filtering — it won't appear on the line plot
    # but gets annotated as a horizontal reference line
    gpu_rows = df[df["variant"] == "gpu"]
    gpu_time = gpu_rows["time_seconds"].iloc[0] if not gpu_rows.empty else None
    ...
    for variant, group in cpu_df.groupby("variant"):
    ...
    max_procs = cpu_df["total_procs"].max()
    ...
    if gpu_time is not None:
        ...
        axes[0].axhline(gpu_speedup, color=COLORS["gpu"], linestyle=":",
                        linewidth=2, label=f"GPU T4 ({gpu_speedup:.1f}×)")
        axes[0].annotate(
            f"GPU T4\n{gpu_speedup:.1f}×",
            xy=(max_procs * 0.55, gpu_speedup),
            xytext=(max_procs * 0.55, gpu_speedup + 0.5),
            color=COLORS["gpu"], fontsize=9, ha="center",
            arrowprops=dict(arrowstyle="->", color=COLORS["gpu"], lw=1.2),
        )
    ...
    axes[0].set_xticks(sorted(cpu_df["total_procs"].unique()))
    ...
    axes[1].set_xticks(sorted(cpu_df["total_procs"].unique()))
...
def plot_time_comparison(df: pd.DataFrame, outpath: str):
    ...
    ax.set_title("K-Means Execution Time — All Configurations (incl. GPU)", fontweight="bold")

    # Legend patches — only for variants present in data
    present = df["variant"].unique()
    patches = [mpatches.Patch(color=COLORS[v], label=LABELS[v])
               for v in ["serial", "omp", "mpi", "mpi_omp", "gpu"] if v in present]
...
def main():
    ...
    # Print GPU summary if present
    gpu_rows = df[df["variant"] == "gpu"]
    if not gpu_rows.empty:
        gpu_time    = gpu_rows["time_seconds"].iloc[0]
        gpu_speedup = serial_time / gpu_time
        print(f"GPU time:        {gpu_time:.5f}s  (speedup: {gpu_speedup:.2f}×)")
    print()

    # ── Plot 1 ──────────────────────────────────────────────────────────────────
    print("Generating speedup & efficiency plot...")
    if len(df[~df["variant"].isin(["serial", "gpu"])]) >= 2:
        plot_speedup_efficiency(df, serial_time, "speedup_efficiency.png")
    else:
        print("  Skipped — need at least 2 parallel CPU rows")
